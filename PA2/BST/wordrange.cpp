...
int main(int argc, char** argv)
{
    ...
    while(getline(input,command)) // get next line of input, store as repeat
    {
        ...
        if(strcmp(op,"w") == 0) // print tree post order
        {
            output << "Width of BST: " + myBST.width() << endl; // write printed tree to file, and add new line
            cout << "Width of BST: ";
            cout << to_string(myBST.width()) << endl;
            continue; // move on to next command
        }

        if(strcmp(op,"lca") == 0) // print tree post order
        {
            //output << "LCA val: " + (myBST.lca(val, 4))->key << endl; // write printed tree to file, and add new line
            cout << "LCA val: " + to_string((myBST.lca(val, 4))->key) << endl;
            continue; // move on to next command
        }
     }
