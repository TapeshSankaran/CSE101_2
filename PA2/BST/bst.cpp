...
int BST :: getWidth(Node* node, int level) {
    if (node == NULL)
        return 0;
...
int BST :: width() {
    ...
    for (int level = 1; ; level++) {
        int levelWidth = getWidth(root, level);
...
Node* BST :: lca(int val1, int val2) {
  ...
  while (curr != nullptr) {
    if (val1 < curr->key && val2 < curr->key) {
      curr = curr->left;
    } else if (val1 > curr->key && val2 > curr->key) {
